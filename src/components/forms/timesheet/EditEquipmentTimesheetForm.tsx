...
interface EditEquipmentTimesheetFormProps {
  onClose: () => void;
  mode: "edit" | "view";
  initialData?: Partial<EquipmentTimesheet>;
  onSubmit: (data: updateEquipmentTimesheetInput) => void;
}

const EditEquipmentTimesheetForm: React.FC<EditEquipmentTimesheetFormProps> = ({
  onClose,
  mode,
  initialData,
  onSubmit,
    ...
    >
      <div className="flex justify-between items-center border-b pb-2 mb-4">
        <h3 className="text-lg font-semibold text-gray-800">
          {mode.charAt(0).toUpperCase() + mode.slice(1)} Equipment Timesheet
        </h3>
        <button
          type="button"
      ...
      <div className="flex items-center space-x-4">
        <Label className="w-32 text-sm font-medium text-gray-700">
          Equipment{!isReadOnly && <span className="text-red-500">*</span>}
        </Label>
        <Controller
          name="equipmentId"
          control={control}
          rules={{ required: !isReadOnly ? "Equipment is required" : false }}
          render={({ field }) => (
            <Select
              disabled={isReadOnly}
...
export default EditEquipmentTimesheetForm;
